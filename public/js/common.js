...
async function getSettings() {
  ...
  try {
    ...
    _settingsCache = {
      ...
      office_address: 'South West Delhi, New Delhi, India',
...
const NAV_ITEMS = [
  ...
  { href: '/properties.html', label: 'Properties' },
  {
    label: 'Localities',
    dropdown: [
      { href: '/uttam-nagar-west.html', label: 'Uttam Nagar West' },
      { href: '/om-vihar.html',         label: 'Om Vihar' },
      { href: '/nawada.html',           label: 'Nawada' },
      { href: '/dwarka-mor.html',       label: 'Dwarka Mor' },
    ]
  },
...
function renderHeader(activePath) {
  ...
  const links = NAV_ITEMS.map(item => {
    if (item.dropdown) {
      const isActive = item.dropdown.some(d => d.href === activePath);
    ...
    <footer class="site-footer">
      <div class="container">
        <div class="footer-grid">
          <div>
            <h4>Prime Builder</h4>
            <p style="max-width:340px;">Helping families in South West Delhi find 2BHK, 3BHK and 4BHK homes that fit their life and budget — with honest advice from first visit to final paperwork.</p>
          ...
          <div>
            <h4>Contact</h4>
            <p>${settings.office_address || 'South West Delhi, New Delhi'}</p>
