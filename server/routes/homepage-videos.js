...
function sortedVideos() {
  return [...store.getAll('homepage_videos')].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
}

// ---------- PUBLIC ----------
router.get('/', (req, res) => {
  res.json(sortedVideos().map(formatVideo));
});

// ---------- ADMIN ----------

// POST /api/homepage-videos
router.post('/', requireAdmin, uploadHomepageVideo, (req, res) => {
  ...
  const existing = store.getAll('homepage_videos');
  ...
  const created = store.insert('homepage_videos', {
...
router.delete('/:id', requireAdmin, async (req, res) => {
  const item = store.getById('homepage_videos', req.params.id);
  ...
  store.remove('homepage_videos', req.params.id);
...
router.put('/reorder', requireAdmin, (req, res) => {
  ...
  order.forEach((id, index) => { store.update('homepage_videos', id, { sort_order: index }); });
  res.json({ success: true });
});
