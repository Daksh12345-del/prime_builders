...
router.post('/', async (req, res) => {
  ...
  const created = store.insert('inquiries', {
    name,
    phone,
    email: email || null,
    budget: budget || null,
  ...
  try {
    const property = property_id ? store.getById('properties', property_id) : null;
...
router.get('/', requireAdmin, (req, res) => {
  const inquiries = store.getAll('inquiries');
  const properties = store.getAll('properties');
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  const existing = store.getById('inquiries', req.params.id);
  if (!existing) return res.status(404).json({ error: 'Inquiry not found.' });

  store.update('inquiries', req.params.id, { status: status || existing.status });
  res.json({ success: true });
});

// DELETE /api/inquiries/:id
router.delete('/:id', requireAdmin, (req, res) => {
  store.remove('inquiries', req.params.id);
  res.json({ success: true });
});
