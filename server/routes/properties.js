...
router.get('/', async (req, res) => {
  try {
    ...
    if (sort === 'price_asc') rows = [...rows].sort((a, b) => (a.down_payment_amount || 0) - (b.down_payment_amount || 0));
    else if (sort === 'price_desc') rows = [...rows].sort((a, b) => (b.down_payment_amount || 0) - (a.down_payment_amount || 0));
...
router.post('/', requireAdmin, async (req, res) => {
  try {
    const {
      title, bhk, locality,
      area_sqft, floor, bathrooms, status,
      is_featured, description, amenities, units_available,
      down_payment_pct, down_payment_amount
    } = req.body;

    if (!title || !bhk || !locality || !area_sqft) {
      return res.status(400).json({ error: 'Title, BHK, locality, and area are required.' });
    }
    ...
    const created = await store.insert('properties', {
      ...
      down_payment_pct: down_payment_pct !== undefined && down_payment_pct !== null ? Number(down_payment_pct) : null,
      down_payment_amount: down_payment_amount !== undefined && down_payment_amount !== null ? Number(down_payment_amount) : null,
      images: [],
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const updates = {
      ...
      down_payment_pct: down_payment_pct !== undefined ? (down_payment_pct === '' || down_payment_pct === null ? null : Number(down_payment_pct)) : existing.down_payment_pct,
      down_payment_amount: down_payment_amount !== undefined ? (down_payment_amount === '' || down_payment_amount === null ? null : Number(down_payment_amount)) : existing.down_payment_amount
    };
